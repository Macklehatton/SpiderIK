@tool
class_name RotationConstraint extends GodotIKConstraint
...
func apply(
	...
	var dir_from_parent = pos_parent_bone.direction_to(pos_bone)
	var dir_to_child = pos_bone.direction_to(pos_child_bone)
	...
	var current_rotation : Quaternion = Quaternion(dir_from_parent, dir_to_child)
	...
	if dir == FORWARD and forward:
		var difference = phi - max_rotation
		var adjustment = Quaternion(rotation_to_initial.get_axis(), difference)
		var child_distance = (pos_child_bone - pos_bone).length()
		adjustment *= dir_to_child * child_distance
		result[2] = pos_bone + adjustment

	if dir == BACKWARD and backward:
		var difference = phi - max_rotation
		var adjustment = Quaternion(-rotation_to_initial.get_axis(), difference)
		var parent_distance = (pos_parent_bone - pos_bone).length()		
		adjustment *= -parent_distance 
		result[0] = pos_bone + adjustment
